...
const User = () => {
  ...
  }, [role, token]); // ✅ add role and token as dependencies
  ...
  const handleEdit = async (userId) => {
    ...
    // try{
    //   const  res = await fetch(`http://localhost:5000/api/users/update-role/${userId}`,{
    //     method: 'PUT',
    //     headers:{
    ...
    userData.forEach((user) => {
      console.log("user.userId:", user.userId, "-> typeof:", typeof user.userId);
    });

    console.log("Searching for userId:", userId, "-> typeof:", typeof userId);

    const foundUser = userData.find((user) =>
      String(user.userId).replace(/"/g, '').trim() ===
      String(userId).replace(/"/g, '').trim()
    );

    console.log("Found User:", foundUser);

    console.log('selected user details: ', foundUser)
        ...
        <div className="container mx-auto">
              ...
              <tbody className="">
                {userData.map((user, index) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-4">
                      <div
                        className={`border flex justify-center p-2 rounded-lg ${user.status === "Active"
          ...
          {isModalOpen && (
            <EditUserModal
              ...
              //   setUsers(
              //     userData.map((user) =>
              //       String(user.userId).replace(/"/g,'').trim() === String(updatedUser.userId).replace(/"/g,'').trim ? updatedUser : user
              //     )
              //   );
          ...
          <div className="mt-4 flex justify-between items-center">
            ...
            <div className="flex space-x-2">
              {Array.from({ length: totalPages }, (_, i) => (
                <button
                  key={i}
                  className={`px-3 py-1 rounded-lg ${i + 1 === currentPage
